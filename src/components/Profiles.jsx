...
function SkeletonCard() {
  return (
    <div className="animate-pulse rounded-2xl border border-border bg-card p-7">
      ...
      <div className="mt-8 h-6 w-28 rounded bg-muted" />
      <div className="mt-2 h-4 w-40 rounded bg-muted" />
      <div className="mt-6 h-14 w-32 rounded-lg bg-muted" />
...
export default function Profiles() {
  const { stats, loading, error } = useDsaStats()

  return (
    <section id="profiles" className="border-y border-border bg-muted/40 py-24">
      <div className="mx-auto max-w-6xl px-6">
        <Reveal>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm text-muted-foreground" style={{ fontFamily: "'Share Tech Mono', monospace" }}>
                Live stats pulled straight from my profiles — consistency you can verify, not claims.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2">
              ...
              <span className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground" style={{ fontFamily: "'Share Tech Mono', monospace" }}>
                {loading ? 'Syncing…' : error ? 'Live cache' : 'Live from API'}
              </span>
            </div>
          </div>
        </Reveal>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {loading
            ...
            : stats?.map((profile, i) => {
                const IconComp = iconMap[profile.name]
                const brand = brandColors[profile.name] || 'currentColor'
                return (
                  <Reveal key={profile.name} delay={i * 0.1} variant="fade">
                    <a
                      href={
                        profile.name === 'LeetCode'
                          ? 'https://leetcode.com/u/venkaiahkalikaya123/'
                          : profile.name === 'CodeChef'
                            ? 'https://www.codechef.com/users/venkey30'
                            : 'https://www.hackerrank.com/profile/venkaiahkalikay1'
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/10"
                    >
                      <span
                      ...
                      <div className="flex items-center justify-between">
                        <span className="flex size-14 items-center justify-center rounded-2xl border border-border bg-muted/60" style={{ color: brand }}>
                      ...
                      <p className="mt-8 text-lg font-bold" style={{ fontFamily: "'Orbitron', sans-serif" }}>
                        {profile.name}
                      </p>
                      <p className="mt-0.5 truncate text-xs text-muted-foreground" style={{ fontFamily: "'Share Tech Mono', monospace" }}>
                        @{profile.username}
                      </p>
                      ...
                      <div className="mt-6 flex items-baseline gap-2">
                        <span
                          ...
                          style={{ fontFamily: "'Orbitron', sans-serif", color: brand }}
                        >
                          <AnimatedNumber value={profile.headline} />
                        </span>
                        <span className="text-xs uppercase tracking-[0.2em] text-muted-foreground" style={{ fontFamily: "'Share Tech Mono', monospace" }}>
